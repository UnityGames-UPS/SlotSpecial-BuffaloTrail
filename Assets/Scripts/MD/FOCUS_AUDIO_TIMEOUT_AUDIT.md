...
|---|---|---|---|---|
| 1 | Visibility listener registration (.jslib + wrapper + Awake) | PASS | `JSFunctCalls.cs:10,41-48`; `SocketIOManager.cs:63-69` | None — matches contract (`OnFocusChanged` lives on the same GameObject that registers the listener). |
| 2 | `OnFocusChanged` public callback | PASS | `SocketIOManager.cs:76-81` | None — `public`, parses `"1"`, routes to both audio (`uiManager.HandleFocusMute`) and socket (`HandleFocusChange`). |
| 3 | Audio mute/unmute honors user sound setting (both paths + invariant) | PASS | `UIManager.cs:963-978`; `AudioController.cs:47-60,166-228`; `SlotBehaviour.cs:409-412` | None — different names (`MuteUnmute`/`CheckFocusFunction`/`m_MutedSound` instead of `SetMuteAll`/`userMuted`) but verified functionally equivalent: focus-regain never unmutes a channel the user muted. |
| 4 | 60s background timeout (`WaitForSecondsRealtime`) | FAIL (partial) → FIXED | `SocketIOManager.cs:103-124`, `402-422` | `FocusTimeoutCheck`'s while-loop didn't re-check `isExiting`/`isBeingDestroyed`, and `CloseSocket()` didn't stop `focusCheckRoutine` — an intentional game-close during a background period could leave a stale coroutine that later fired a bogus disconnect popup. Added the guard to the while condition + early-exit, and stop the routine from `CloseSocket()`. |
| 5 | `OnError` (session-expired vs generic) | PASS | `SocketIOManager.cs:224,271-288` | None — matches reference exactly. |
| 6 | Instant JS-side mute (WebAudio suspend in `.jslib`) | MISSING → FIXED | `Assets/Plugins/webgl/CustomJsLib.jslib:57-97` | Added `setUnityAudioSuspended` helper, called as the first step of `sendFocusToUnity` (suspends on blur, resumes on focus) before the `SendMessage` handoff — matches the reference contract. |
