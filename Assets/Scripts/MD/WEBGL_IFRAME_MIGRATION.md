...
<head>
  ...
  <!-- Locked viewport: no user zoom, cover the notch. -->
  <meta name="viewport"
  ...
  <style>
    ...
    body {
      margin: 0;
      padding: 0;
      /* fixed + no overscroll: stop iOS rubber-band scrolling */
      position: fixed;
      inset: 0;
      width: 100%;
      height: 100%;
...
- The `console.*` overrides silence production logs; drop that block if the game needs console output.
